class ABNTRulesEngine {
  ...
  checkFontSize(fontSize, expectedSize, tolerance = 0.5) {
    ...
    if (actual === null)
      return { valid: false, actual: null, expected: expectedSize };
    ...
    return {
      ...
      difference: actual - expectedSize,
    };
  }

  checkFont(fontFamily, allowedFonts) {
    if (!fontFamily) {
      // FIX: Retorne 'allowedFonts' mesmo em caso de falha.
      return { valid: false, actual: null, allowed: allowedFonts };
    }
  ...
  checkLineSpacing(lineHeight, expectedPercent, tolerance = 5) {
    if (!lineHeight)
      return { valid: false, actual: null, expected: expectedPercent };
    ...
    if (actualPercent === null)
      return { valid: false, actual: lineHeight, expected: expectedPercent };
  ...
  validateGeneralFormatting(element) {
    ...
    const fontSizeCheck = this.checkFontSize(
      element.styles.fontSize,
      this.rules.GERAL.TAMANHO_TEXTO,
    );
    scores.max += 10;
    if (fontSizeCheck.valid) {
      ...
      issues.push({
        ...
        actual: fontSizeCheck.actual
          ? `${fontSizeCheck.actual}pt`
          : "não definido",
        severity: "medium",
      });
    }

    const fontCheck = this.checkFont(
      element.styles.fontFamily,
      this.rules.GERAL.FONTES,
    );
    scores.max += 10;
    if (fontCheck.valid) {
      scores.earned += 10;
  ...
  validateDevelopmentFormatting(element) {
    ...
    const spacingCheck = this.checkLineSpacing(
      element.styles.lineHeight,
      this.rules.DESENVOLVIMENTO.ESPACAMENTO,
    );
    scores.max += 10;
    ...
    const alignCheck = this.checkAlignment(
      element.styles.textAlign,
      this.rules.DESENVOLVIMENTO.ALINHAMENTO_CORRETO,
    );
    scores.max += 10;
    if (alignCheck.valid) {
      scores.earned += 10;
    ...
    if (element.styles.textIndent) {
      const indentCheck = this.checkIndent(
        element.styles.textIndent,
        this.rules.DESENVOLVIMENTO.RECUO,
      );
      scores.max += 10;
  ...
  validateReferenceFormatting(reference) {
    ...
    const spacingCheck = this.checkLineSpacing(
      reference.styles.lineHeight,
      this.rules.REFERENCIAS.ESPACAMENTO,
    );
    scores.max += 5;
    ...
    const alignCheck = this.checkAlignment(
      reference.styles.textAlign,
      this.rules.REFERENCIAS.ALINHAMENTO,
    );
    scores.max += 5;
    if (alignCheck.valid) {
      scores.earned += 5;
