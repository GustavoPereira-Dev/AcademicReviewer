...
class ReviewOrchestrator {
  ...
  async parseDocument() {
    ...
    console.log(
      `✅ Documento carregado: ${this.documentData.sections.length} seções, ${this.documentData.paragraphs.length} parágrafos`,
    );
  }
  ...
  detectDocumentType() {
    ...
    //this.documentData.metadata.map((h) => console.log("A " + h.text));
  ...
  setDocumentType(type) {
    ...
    if (!validTypes.includes(type)) {
      throw new Error(
        `Tipo de documento inválido: ${type}. Tipos válidos: ${validTypes.join(", ")}`,
      );
    }
    this.documentType = type;
    console.log(
      `📝 Tipo de documento definido manualmente: ${type.toUpperCase()}`,
    );
  }
  ...
  async review(documentType = null) {
    ...
    switch (this.documentType) {
      ...
      default:
        throw new Error(
          `Tipo de documento não suportado: ${this.documentType}`,
        );
    }
  ...
  async getDocumentStatistics() {
    ...
    return {
      ...
      totalWords: this.documentData.paragraphs.reduce(
